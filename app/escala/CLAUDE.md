...
`POST /escala/excluir-varias` (`excluir_varias`) — exclusão em lote a partir de `ministerio/detalhe.html`: checkboxes (`name="escala_ids"`) só aparecem nas escalas manuais (`plantao_turno_id is None`); a rota também ignora qualquer id que ainda assim chegue com `plantao_turno_id` preenchido — geradas por rodízio ficam fora do escopo dessa ação, de propósito (excluir uma ocorrência de rodízio isoladamente não é o fluxo esperado). Cada id é filtrado por `_eh_lider_do_ministerio` individualmente, nunca confia na lista recebida. Mesma UI genérica de "modo seleção" (`app/static/js/main.js`, `data-selecao-*`) usada por `comunidade.excluir_varias`.
