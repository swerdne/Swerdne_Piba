...
- **Gerada por rodízio** — criada e mantida automaticamente pelo motor de sincronização de [app/plantao](../plantao/CLAUDE.md) (`plantao_turno_id`/`plantao_periodo` preenchidos). Sempre com exatamente 1 `Funcao`. Ver `Escala.plantao_fixado` abaixo e o CLAUDE.md de `plantao` para os detalhes do motor — este módulo só precisa saber que essas `Escala` existem e são, pra todo o resto (calendário, relatório, notificação), indistinguíveis de uma manual.
...
`POST /escala/funcao/<id>/mover` usa `trocar_atribuicao(funcao_a, funcao_b)` (`models.py`) para o **swap completo** (membro + status + notificado_em) entre duas `Funcao` — não é "mover uma pessoa para uma vaga vazia", é sempre troca de duas posições. A mesma `trocar_atribuicao` é reaproveitada por `plantao.sincronizacao.marcar_ausencia` (troca entre a `Funcao` de um período e a do seguinte, em duas `Escala` diferentes) — se mudar a semântica do swap, mude num lugar só.
...
- "Registrar ausência" nessas escalas não usa o fluxo genérico — é `plantao.registrar_ausencia_na_escala`, que resolve turno/período a partir da `Escala` e delega para `plantao.sincronizacao.marcar_ausencia` (ver [app/plantao/CLAUDE.md](../plantao/CLAUDE.md)).
