...
// {urlConcluir, csrf, passos: [{seletor, titulo, texto}]} -- seletor null/
// ausente = passo centralizado (sem destacar elemento nenhum), usado pro
// primeiro e ultimo passo. Ver comunidade/detalhe.html pro primeiro uso.
(function iniciarTutorialSpotlight() {
    var dadosEl = document.getElementById("tutorial-dados");
    if (!dadosEl) return; // pagina sem tutorial, ou usuario ja viu (ver server-side)
    ...
    function irParaPasso(indice, semScroll) {
        ...
        function aplicar() {
            if (alvo) {
                ...
                // Passo sem elemento (boas-vindas/conclusao): destaque "no meio",
                // do tamanho da propria janela, ou seja, sem cutout visivel.
